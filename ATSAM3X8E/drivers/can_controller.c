 ...
 */
uint8_t can_init_def_tx_rx_mb(uint32_t can_br) { return can_init(can_br, 1, 2); }
...
uint8_t can_init(uint32_t can_br, uint8_t num_tx_mb, uint8_t num_rx_mb) {
    ...
    PMC->PMC_PCR = PMC_PCR_EN | (0 << PMC_PCR_DIV_Pos) | PMC_PCR_CMD |
                   (ID_CAN0 << PMC_PCR_PID_Pos);  // DIV = 1(can clk = MCK/2), CMD = 1 (write), PID = 2B (CAN0)
    ...
    for (int n = num_tx_mb; n <= num_rx_mb;
...
uint8_t can_send(CAN_MESSAGE* can_msg, uint8_t tx_mb_id) {
    ...
    if (CAN0->CAN_MB[tx_mb_id].CAN_MSR & CAN_MSR_MRDY) {
        ...
        CAN0->CAN_MB[tx_mb_id].CAN_MDL =
            can_msg->data[3] << 24 | can_msg->data[2] << 16 | can_msg->data[1] << 8 | can_msg->data[0];
        CAN0->CAN_MB[tx_mb_id].CAN_MDH =
            can_msg->data[7] << 24 | can_msg->data[6] << 16 | can_msg->data[5] << 8 | can_msg->data[4];
